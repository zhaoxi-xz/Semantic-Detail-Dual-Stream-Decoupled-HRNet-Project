...
from Models.SDHRNet import MultiScaleSemanticBranch, DilatedDetailBranch, adaptive_fusion
...
class SDDeepLabv3plus(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, _print=True):
        if _print:
            print("Constructing SD-DeepLabv3+ model...")
            print("Backbone: Xception")
            print("Number of classes: {}".format(n_classes))
            print("Output stride: {}".format(os))
            print("Number of Input Channels: {}".format(nInputChannels))
        super(SDDeepLabv3plus, self).__init__()

        # Atrous Conv
        self.xception_features = Xception(nInputChannels, os)

        # 语义-细节分支
        self.semantic_branch = MultiScaleSemanticBranch(in_channels=128, out_channels=128)
        self.detail_branch = DilatedDetailBranch(in_channels=128, out_channels=128)
        
        # 特征融合层
        self.shape_fusion = nn.Sequential(nn.Conv2d(128 + 128, 128, kernel_size=1, bias=False),
                                       nn.BatchNorm2d(128),
    ...
    def forward(self, input):
        ...
        # 应用语义-细节分支
        semantic_feat = self.semantic_branch(low_level_features)
        detail_feat = self.detail_branch(low_level_features)
        
        # 融合语义-细节特征并与原始特征结合
        fused_shape = adaptive_fusion(semantic_feat, detail_feat)
        fused_shape = self.shape_fusion(torch.cat([semantic_feat, detail_feat], dim=1))
        low_level_features_enhanced = low_level_features + fused_shape  # 残差连接
        ...
        low_level_features_enhanced = self.conv2(low_level_features_enhanced)
        low_level_features_enhanced = self.bn2(low_level_features_enhanced)
        low_level_features_enhanced = self.relu(low_level_features_enhanced)

        x = torch.cat((x, low_level_features_enhanced), dim=1)
        x = self.last_conv(x)
...
if __name__ == "__main__":
    model = SDDeepLabv3plus(nInputChannels=3, n_classes=12, os=16, _print=True)
    model.eval()

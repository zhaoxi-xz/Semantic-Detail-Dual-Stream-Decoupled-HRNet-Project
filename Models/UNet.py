...
class expansive_block(nn.Module):
    ...
    def forward(self, e, d):
        d = self.up(d)
        # 更精确的尺寸匹配，确保特征图能够正确拼接
        target_size = d.size()[2:]  # 获取上采样后的目标尺寸
        if e.size()[2:] != target_size:
            # 使用插值调整encode特征图的尺寸以匹配decode特征图
            e = nn.functional.interpolate(e, size=target_size, mode='bilinear', align_corners=True)
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 添加上采样操作，将输出特征图恢复到与目标标签相同的分辨率(512x512)
        final_layer = nn.functional.interpolate(final_layer, size=(512, 512), mode='bilinear', align_corners=True)
        return final_layer
